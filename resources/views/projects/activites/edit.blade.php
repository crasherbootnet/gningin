...
<style type="text/css">
	...
	<div class="content">
        <div class="container-fluid">
            <div class="row">
				<!-- multistep form -->
		<form id="msform" action="{{ url('projects/'.$project->short_code.'/activites/update/'.$activite->short_code) }}" method="POST" enctype="multipart/form-data" class="from-group">
			...
			<fieldset>
				...
				<input type="text" name="libelle" placeholder="Libelle" value="{{ $activite->libelle }}"/>
			...
			<fieldset class="fieldset-description-activite">
				...
				<textarea cols="20" rows="18" name="content">{{ $activite->content }}</textarea>
				<div class="row group-date">
					<div class="col-md-4">
						<label>Date de debut de l'activité (*)</label>
						<input type="date" name="date_debut" class="form-control" value="@php echo explode(' ', $activite->date_debut)[0]; @endphp" required>
					</div>
					<div class="col-md-4">
			...
			<fieldset>
				...
				<textarea cols="20" rows="18" name="rapport_moral">{{ $activite->rapport_moral }}</textarea>
				<a href="{{ url('projects/activites/'.$project->short_code) }}" class="btn btn-annuler">Annuler</a>
				<input type="button" name="previous" class="previous action-button" value="Previous" />
				<input type="button" name="next" class="next action-button" value="Next" />
			</fieldset>
			<fieldset>
				...
				<textarea cols="20" rows="18" name="rapport_financier">{{ $activite->rapport_financier }}</textarea>
				<a href="{{ url('projects/activites/'.$project->short_code) }}" class="btn btn-annuler">Annuler</a>
				<input type="button" name="previous" class="previous action-button" value="Previous" />
				<input type="button" name="next" class="next action-button" value="Next" />
			</fieldset>
			<fieldset>
				...
				<div class="alert-primary" role="alert" @if($activite->liste_presence == 0) hidden @endif>
					Enregistrer les personnes présentes à l'activité !!!
				</div>
					
					<div class="row">
						<table class="table table-bordered col-md-12" @if($activite->liste_presence == 0) "hidden" @endif>
							<thead>
								<tr>
									<th>Id</th>
									<th>Nom && Prénom</th>
