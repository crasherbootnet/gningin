            ...
            <ul aria-expanded="false" class="collapse">
                @foreach($projectHist->projectshistorisations as $historisation)
                    @if($projectHistorisation->id == $historisation->id)
                        <li class="active"><a href="{{ url('projects/historisation-context/'.$historisation->id) }}">{{ $historisation->created_at }}</a></li>
                    @else
                        <li><a href="{{ url('projects/historisation-context/'.$historisation->id) }}">{{ $historisation->created_at }}</a></li>
                    @endif
                @endforeach
            </ul>
        </li>
...
@section('content')
	<div class="content">
        <div class="container-fluid">
            <div class="row">
				<form action="{{ url('bayeurs/projects/historisation-context/'.$projectHistorisation->id) }}" method="POST" enctype="multipart/form-data">
					<input type="text" name="_token" value="{{ csrf_token() }}" hidden>
					<textarea rows="12" cols="100" name="content" class="tinymce">
						@if($context && $context->content)  {{ $context->content }} @endif
					</textarea>
					<div class="row" style="margin: 10px 0px 0px 0px">
						<a href="{{ url('bayeurs/projects/historisations-versions/'.$projectHistorisation->short_code) }}" class="btn btn-annuler">retoour</a>
					</div>
				</form>
			</div>
	    </div>
