...
<ul id="sidebarnav">
    <li> 
        <a class="has-arrow waves-effect waves-dark" href="{{ url('bayeurs/projects/historisation-objectif/'.$projectHistorisation->id) }}" aria-expanded="false">
            ...
            <ul aria-expanded="false" class="collapse">
                @foreach($projectHist->projectshistorisations as $historisation)
                    @if($projectHistorisation->id == $historisation->id)
                        <li class="active"><a href="{{ url('bayeurs/projects/historisation-objectif/'.$historisation->id) }}">{{ $historisation->created_at }}</a></li>
                    @else
                        <li><a href="{{ url('bayeurs/projects/historisation-objectif/'.$historisation->id) }}">{{ $historisation->created_at }}</a></li>
                    @endif
                @endforeach
            </ul>
        </li>
...
@section('content')
	<div class="content">
        <div class="container-fluid">
            <div class="row">
				<form action="{{ url('bayeurs/projects/historisation-objectif/'.$projectHistorisation->id) }}" method="POST" enctype="multipart/form-data">
